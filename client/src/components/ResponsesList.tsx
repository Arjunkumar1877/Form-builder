...
const FormResponsesDisplay = () => {
  ...
  const formId = query.get('formId') || "66f56a5ca73d0a037bcacce5"; // Use query param if available
            ...
            {response.responses.map(({ key, value, _id }) => (
              <div key={_id} className="mb-2 flex items-center">
                ...
                {typeof value === 'string' && value.includes('http') && 
                  (value.startsWith('https://') || value.startsWith('.png') || value.startsWith('.jpeg')) ? (
                  <div className="flex items-center mt-1">
                    <img src={value} alt={key} className="w-12 h-12 rounded-md object-cover mr-2" />
                    {/* <span className="text-blue-600 hover:underline cursor-pointer" onClick={() => handleCopyLink(value)}>
                      {value}
                    </span> */}
                    <button
